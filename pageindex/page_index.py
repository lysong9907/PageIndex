...
def toc_detector_single_page(content, model=None):
    ...
    if result is None:
        # Fallback: check for TOC-like patterns
        result = bool(re.search(r'\b(contents|directory|index|table of contents|chapter|section)\b', response, re.IGNORECASE))
    return result
...
def check_if_toc_extraction_is_complete(content, toc, model=None):
    ...
    response = ChatGPT_API(model=model, prompt=prompt)
    json_content = extract_json(response)
    return json_content.get('completed', False)
...
def detect_page_index(toc_content, model=None):
    ...
    response = ChatGPT_API(model=model, prompt=prompt)
    json_content = extract_json(response)
    result = json_content.get('page_index_given_in_toc', '')
    if result not in ('yes', 'no'):
        # Fallback: heuristic detection
        result = 'yes' if re.search(r'\b(page|p\.?|pg\.?)\s*\d+', toc_content, re.IGNORECASE) else 'no'
        print(f'[detect_page_index] LLM returned invalid response, using heuristic fallback: {result}')
    return result
...
def single_toc_item_index_fixer(section_title, content, model="gpt-4o-2024-11-20"):
    ...
    response = ChatGPT_API(model=model, prompt=prompt)
    json_content = extract_json(response)    
    pi = json_content.get('physical_index')
    if pi is None:
        return None
    return convert_physical_index_to_int(pi)
